/* =========================================================================
   Gesipan — whiteboard infinito (minimal) + bookmarks
   Vanilla JS, sin dependencias. v5
   ========================================================================= */
...
const state = {
  ...
  cam: { x: 0, y: 0, zoom: 1 },
  tool: "pan",           // pan | group | connect | resize
  noteStyle: "postit", activeColor: "yellow",
  connectAnchorFrom: null,   // { noteId, anchor, x, y }
  connecting: null,          // { fromX, fromY } arrastrando una raya nueva
...
async function loadBoard(id) {
  ...
  state.connectAnchorFrom = null; state.connecting = null;
  state.selectedNoteId = null; state.selectedGroupId = null;
  hideInspector();
  renderBoardList();
  renderAll();
  applyFilter(); applyPrivacy(); applyToolMode();
}
...
function startGroupDrag(e, g, div) {
  ...
  const offX = w.x - g.x, offY = w.y - g.y;
  state.drag = { kind: "group", id: g.id, offX, offY };
  div.classList.add("dragging");
  const onMove = (ev) => {
    const w = screenToWorld(ev.clientX, ev.clientY);
    const dx = w.x - state.drag.offX - g.x, dy = w.y - state.drag.offY - g.y;
    g.x += dx; g.y += dy;
    div.style.left = g.x + "px"; div.style.top = g.y + "px";
...
function createNoteEl(n) {
  ...
  if (n.group_id) { const a = document.createElement("span"); a.className = "note-anchored"; a.textContent = "📌"; a.title = "Anclada al grupo"; div.appendChild(a); }
  ...
  // Botón redimensionar (esquina inferior derecha)
  const rs = document.createElement("div");
  rs.className = "resize-handle";
  rs.addEventListener("pointerdown", (e) => { e.preventDefault(); e.stopPropagation(); startNoteResize(e, n, div); });
  div.appendChild(rs);

  // Botón anclar a grupo (aparece al pasar sobre un grupo)
  const anchorBtn = document.createElement("button");
  anchorBtn.className = "anchor-group-btn hidden";
  anchorBtn.textContent = "📌 Anclar al grupo";
  anchorBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    const g = groupUnder(n);
    if (g) { anchorToGroup(n, g.id); }
  });
  ...
  actions.appendChild(actionBtn("⬆", "Al frente", () => raiseNote(n, div)));
...
async function anchorToGroup(n, groupId) {
  n.group_id = groupId;
  await api(`/api/notes/${n.id}`, "PATCH", { group_id: groupId });
  renderAll(); applyFilter(); applyPrivacy(); applyToolMode();
}
async function unanchorFromGroup(n) {
  n.group_id = null;
  await api(`/api/notes/${n.id}`, "PATCH", { group_id: null });
  renderAll(); applyFilter(); applyPrivacy(); applyToolMode();
}
...
function attachNoteEvents(div, n, anchorBtn) {
  div.addEventListener("pointerdown", (e) => {
    ...
    if (e.target.classList.contains("anchor") || e.target.classList.contains("resize-handle")) return;
  ...
  // Al soltar sobre un grupo, mostrar botón de anclar
  div.addEventListener("pointerup", () => {
    const g = groupUnder(n);
    if (g && n.group_id !== g.id) { anchorBtn.classList.remove("hidden"); anchorBtn.textContent = "📌 Anclar a: " + (g.title || "grupo"); }
    else if (!g) anchorBtn.classList.add("hidden");
  });
}
...
function startNoteDrag(e, note, div) {
  const r = div.getBoundingClientRect();
  const w = screenToWorld(r.left, r.top);
  const offX = w.x - note.x, offY = w.y - note.y;
  ...
  const onMove = (ev) => {
    ...
    const w = screenToWorld(ev.clientX, ev.clientY);
    note.x = w.x - state.drag.offX; note.y = w.y - state.drag.offY;
    div.style.left = note.x + "px"; div.style.top = note.y + "px";
    // Resaltar grupo bajo la nota
  ...
  const onUp = () => {
    ...
    if (moved) { noteJustDragged = true; saveNote(note); dropAnimation(div); }
  };
  window.addEventListener("pointermove", onMove); window.addEventListener("pointerup", onUp);
}

// Redimensionar nota (esquina inferior derecha)
function startNoteResize(e, note, div) {
  const sx = e.clientX, sy = e.clientY, sw = note.width, sh = note.height;
  const onMove = (ev) => {
    note.width = Math.max(120, sw + (ev.clientX - sx) / state.cam.zoom);
    note.height = Math.max(80, sh + (ev.clientY - sy) / state.cam.zoom);
    div.style.width = note.width + "px"; div.style.height = note.height + "px";
    drawConnections();
  };
  const onUp = () => {
    window.removeEventListener("pointermove", onMove); window.removeEventListener("pointerup", onUp);
...
// Animación orgánica de drop (bounce suave)
function dropAnimation(div) {
  div.classList.remove("drop-anim");
  void div.offsetWidth; // reinicia la animación
  div.classList.add("drop-anim");
}

// ============================ CONEXIONES (arrastrar raya) ============================
// Desde un punto de anclaje: arrastrar hasta otro ancla (o soltar en nota).
function startAnchorDrag(n, anchor, e) {
  ...
  state.connecting = {
    ...
    toX: e.clientX - el.board.getBoundingClientRect().left,
    toY: e.clientY - el.board.getBoundingClientRect().top,
  };
  document.querySelectorAll(".anchor").forEach(a => a.classList.remove("selected-anchor"));
  el.world.querySelectorAll(".note").forEach(d => {
    if (d.dataset.id == n.id) d.querySelector(`.anchor-${anchor}`).classList.add("selected-anchor");
  });

  const onMove = (ev) => {
    const r = el.board.getBoundingClientRect();
    state.connecting.toX = ev.clientX - r.left;
    state.connecting.toY = ev.clientY - r.top;
    drawConnecting();
  };
  const onUp = (ev) => {
    window.removeEventListener("pointermove", onMove); window.removeEventListener("pointerup", onUp);
    state.connecting = null;
    // ¿Aterrizamos en un anchor de otra nota?
    const t = document.elementFromPoint(ev.clientX, ev.clientY);
    const targetAnchor = t ? t.closest(".anchor") : null;
    if (targetAnchor && Number(targetAnchor.dataset.noteId) !== n.id) {
      createConnection(n.id, anchor, Number(targetAnchor.dataset.noteId), targetAnchor.dataset.anchor);
    } else {
      // Si soltamos sobre una nota (no ancla), conectar centro->centro
      const noteEl = t ? t.closest(".note") : null;
      if (noteEl && Number(noteEl.dataset.id) !== n.id) {
        createConnection(n.id, anchor, Number(noteEl.dataset.id), "center");
      }
    }
...
function createConnection(fromId, fromAnchor, toId, toAnchor) {
  api(`/api/boards/${state.currentBoardId}/connections`, "POST", {
    from_id: fromId, to_id: toId, from_anchor: fromAnchor, to_anchor: toAnchor, label: "",
...
function drawConnections() {
  ...
  defs.innerHTML = `<marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="var(--accent)"/></marker>`;
...
el.board.addEventListener("pointerdown", (e) => {
  ...
  if (state.tool === "group") { const w = screenToWorld(e.clientX, e.clientY); createGroup(w.x, w.y); return; }
...
function viewCenterScreen() { const r = el.board.getBoundingClientRect(); return { x: r.left + r.width / 2, y: r.top + r.height / 2 }; }

// Detail grande al crear: centra y amplía la nota temporalmente
function openNoteDetail(note) {
  const noteEl = el.world.querySelector(`.note[data-id="${note.id}"]`);
  if (!noteEl) return;
  // Centra la cámara en la nota
...
function raiseNote(n, div) {
  api(`/api/notes/${n.id}/raise`, "POST").then(() => { n.z = Math.max(...state.notes.map(x => x.z)) + 1; div.style.zIndex = n.z; });
}
...
function openInspector(n) {
  ...
  el.inspText.value = n.text; el.inspTags.value = n.tags;
  ...
  if (grpBtn) {
    ...
    grpBtn.textContent = g ? `📌 ${g.title || "Grupo"} (clic para desanclar)` : "📌 Anclar a grupo";
    grpBtn.classList.toggle("active", !!g);
    grpBtn.onclick = () => {
      if (n.group_id) { unanchorFromGroup(n); openInspector(n); }
      else { const g = groupUnder(n); if (g) { anchorToGroup(n, g.id); openInspector(n); } }
    };
  }
  el.inspText.focus();
}
...
el.inspText.addEventListener("input", () => {
  ...
  const ne = el.world.querySelector(`.note[data-id="${editingNote.id}"]`);
  if (ne) ne.querySelector(".content").textContent = editingNote.text;
...
el.inspStylePostit.addEventListener("click", () => { if (!editingNote) return; editingNote.style = "postit"; renderAll(); applyFilter(); applyPrivacy(); applyToolMode(); saveNote(editingNote); });
el.inspStylePin.addEventListener("click", () => { if (!editingNote) return; editingNote.style = "pin"; renderAll(); applyFilter(); applyPrivacy(); applyToolMode(); saveNote(editingNote); });
...
function setTool(tool) {
  ...
  if (tool !== "connect") { state.connectAnchorFrom = null; state.connecting = null; }
  applyToolMode();
}
function applyToolMode() {
  const cm = state.tool === "connect";
  el.world.querySelectorAll(".note").forEach(d => d.classList.toggle("anchor-mode", cm));
}
function setNoteStyle(s) {
  state.noteStyle = s;
  document.getElementById("postit-btn").classList.toggle("active", s === "postit");
  document.getElementById("pin-btn").classList.toggle("active", s === "pin");
}
...
document.getElementById("group-btn").addEventListener("click", () => setTool(state.tool === "group" ? "pan" : "group"));
document.getElementById("connect-btn").addEventListener("click", () => setTool(state.tool === "connect" ? "pan" : "connect"));
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Delete" && !e.target.isContentEditable) {
    if (state.selectedNoteId !== null) { const n = state.notes.find(x => x.id === state.selectedNoteId); if (n) deleteNote(n); }
    else if (state.selectedGroupId !== null) { const g = state.groups.find(x => x.id === state.selectedGroupId); if (g) deleteGroup(g); }
  }
});
...
// ============================ BOOKMARKS ============================
const bmState = { collections: [], bookmarks: [], currentCollection: null, search: "" };
const bmEl = {
  ...
  grid: document.getElementById("bm-grid"), empty: document.getElementById("bm-empty"),
  modal: document.getElementById("bm-modal"), url: document.getElementById("bm-url"),
  titleInput: document.getElementById("bm-title"), excerpt: document.getElementById("bm-excerpt"),
  note: document.getElementById("bm-note"), tags: document.getElementById("bm-tags"),
...
function renderBookmarks() {
  ...
  bmEl.title.textContent = bmState.currentCollection === null ? "Todos los bookmarks" : (bmState.collections.find(c => c.id === bmState.currentCollection)?.name || "Bookmarks");
...
function createBookmarkCard(b) {
  ...
  if (b.thumbnail) { const img = document.createElement("img"); img.src = b.thumbnail; img.alt = ""; img.loading = "lazy"; img.onerror = () => img.remove(); thumb.appendChild(img); }
  else if (b.favicon) { const fav = document.createElement("img"); fav.className = "bm-favicon"; fav.src = b.favicon; fav.alt = ""; fav.onerror = () => fav.remove(); thumb.appendChild(fav); }
  ...
  thumb.appendChild(star); card.appendChild(thumb);
  ...
  const a = document.createElement("a"); a.href = b.url; a.target = "_blank"; a.rel = "noopener"; a.textContent = b.title || b.url;
  ...
  const host = document.createElement("div"); host.className = "bm-host"; host.textContent = hostOf(b.url); body.appendChild(host);
  if (b.tags.trim()) { const tags = document.createElement("div"); tags.className = "bm-tags"; b.tags.split(",").map(t => t.trim()).filter(Boolean).forEach(t => { const s = document.createElement("span"); s.className = "bm-tag"; s.textContent = t; tags.appendChild(s); }); body.appendChild(tags); }
  const del = document.createElement("button"); del.className = "bm-del"; del.textContent = "✕ eliminar";
  del.addEventListener("click", async () => { await api(`/api/bookmarks/${b.id}`, "DELETE"); loadBookmarks(); });
  body.appendChild(del); card.appendChild(body);
  return card;
}
...
function openBmModal() {
  ...
  bmEl.url.value = ""; bmEl.titleInput.value = ""; bmEl.excerpt.value = ""; bmEl.note.value = ""; bmEl.tags.value = ""; bmEl.fetchStatus.textContent = "";
...
bmEl.fetchBtn.addEventListener("click", async () => {
  ...
  try { const m = await api("/api/bookmarks/fetch", "POST", { url }); if (!bmEl.titleInput.value) bmEl.titleInput.value = m.title; if (!bmEl.excerpt.value) bmEl.excerpt.value = m.excerpt; bmEl.fetchStatus.textContent = "✓ Metadatos descubiertos"; }
...
bmEl.save.addEventListener("click", async () => {
  ...
  try {
    await api("/api/bookmarks", "POST", { url, collection_id: cid, title: bmEl.titleInput.value.trim(), excerpt: bmEl.excerpt.value.trim(), note: bmEl.note.value.trim(), tags: bmEl.tags.value.trim() });
...
function setListView(enabled) {
  ...
  const b = document.getElementById("view-toggle"); if (b) b.textContent = enabled ? "▦" : "☰";
  localStorage.setItem("gesipan-listview", enabled ? "1" : "0");
  if (!bmEl.view.classList.contains("hidden")) renderBookmarks();
}
...
function trashRow(label, onRestore, onPurge) {
  ...
  const restore = document.createElement("button"); restore.className = "ghost-btn small"; restore.textContent = "↩ Restaurar";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const savedL = localStorage.getItem("gesipan-listview") === "1"; setListView(savedL);
  document.getElementById("dark-toggle").addEventListener("click", () => applyTheme());
  document.getElementById("view-toggle").addEventListener("click", () => setListView(!state.listView));
