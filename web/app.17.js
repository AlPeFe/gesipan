...
function attachNoteEvents(div, n, anchorBtn) {
  ...
  div.addEventListener("click", e => {
    ...
    // Al seleccionar la nota: si pertenece a un grupo, mostrar "Desanclar" en la
    // propia nota (detectado por n.group_id, sin superponerse al grupo).
    if (n.group_id) {
      anchorBtn.classList.remove("hidden");
      anchorBtn.textContent = "📌 Desanclar";
    } else {
  ...
  div.addEventListener("pointerup", () => {
    // Nota SIN grupo: si la sueltas sobre un grupo, ofrece "Anclar".
    if (!n.group_id) {
      const g = groupUnder(n);
      if (state.selectedNoteId === n.id && g) {
        anchorBtn.classList.remove("hidden");
        anchorBtn.textContent = "📌 Anclar a " + (g.title || "grupo");
      } else {
        anchorBtn.classList.add("hidden");
      }
    }
...
function hideInspector() {
  ...
  setSelections();
  // Y el botón de desanclar/anclar deja de mostrarse (ya no hay foco).
  el.world.querySelectorAll(".anchor-group-btn").forEach(b => b.classList.add("hidden"));
}
