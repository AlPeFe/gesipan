...
// Elimina (mueve a papelera) una nota.
async function deleteNote(n) {
  await api(`/api/notes/${n.id}`, "DELETE"); // soft-delete
...
function createBookmarkCard(b) {
  ...
  del.addEventListener("click", async () => {
    await api(`/api/bookmarks/${b.id}`, "DELETE"); // soft-delete -> papelera
    loadBookmarks();
  });
...
document.addEventListener("DOMContentLoaded", () => {
  const saved = localStorage.getItem("gesipan-theme");
...
// ---------------------------------------------------------------------------
// VISTA LISTA / TARJETAS (notas y bookmarks)
// ---------------------------------------------------------------------------
function setListView(enabled) {
  ...
  // Re-render del grid de bookmarks si está visible.
  if (!bmEl.view.classList.contains("hidden")) renderBookmarks();
}
document.addEventListener("DOMContentLoaded", () => {
  const saved = localStorage.getItem("gesipan-listview") === "1";
  setListView(saved);
  const btn = document.getElementById("view-toggle");
  if (btn) btn.addEventListener("click", () => setListView(!state.listView));
});
...
async function openTrash() {
  ...
  const notesEl = document.getElementById("trash-notes");
  const bmsEl = document.getElementById("trash-bookmarks");
  const emptyEl = document.getElementById("trash-empty");
  notesEl.innerHTML = "";
  bmsEl.innerHTML = "";
  emptyEl.classList.toggle("hidden", trash.notes.length + trash.bookmarks.length > 0);

  if (trash.notes.length) {
    notesEl.appendChild(header("Notas"));
    for (const n of trash.notes) {
      notesEl.appendChild(trashRow(n.text || "(sin texto)", () => restoreNote(n), () => purgeNote(n)));
    }
  }
  if (trash.bookmarks.length) {
    bmsEl.appendChild(header("Bookmarks"));
    for (const b of trash.bookmarks) {
      bmsEl.appendChild(trashRow(b.title || b.url, () => restoreBookmark(b), () => purgeBookmark(b)));
    }
  }
...
function trashRow(label, onRestore, onPurge) {
  ...
  restore.className = "ghost-btn small"; restore.textContent = "↩ Restaurar";
  restore.addEventListener("click", async () => { await onRestore(); openTrash(); });
  const purge = document.createElement("button");
  purge.className = "danger-btn small"; purge.textContent = "🗑 Borrar";
  purge.addEventListener("click", async () => { await onPurge(); openTrash(); });
  row.append(span, restore, purge);
  return row;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (trashClose) trashClose.addEventListener("click", () => document.getElementById("trash-modal").classList.add("hidden"));
});

// ---------------------------------------------------------------------------
// ORDENAR + EXPORT BOOKMARKS + CLASIFICAR
// ---------------------------------------------------------------------------
document.addEventListener("DOMContentLoaded", () => {
  // Ordenar bookmarks por fecha (asc/desc).
  let bmSortAsc = true;
  const sortBtn = document.getElementById("bm-sort-btn");
  if (sortBtn) sortBtn.addEventListener("click", () => {
    bmSortAsc = !bmSortAsc;
    const sorted = [...bmState.bookmarks].sort((a, b) =>
      bmSortAsc ? a.created_at.localeCompare(b.created_at) : b.created_at.localeCompare(a.created_at)
    );
    bmState.bookmarks = sorted;
    renderBookmarks();
  });

  // Export bookmarks a Markdown.
  const bmExport = document.getElementById("bm-export-btn");
  if (bmExport) bmExport.addEventListener("click", () => {
    window.open("/api/bookmarks/export.md", "_blank");
  });
});
