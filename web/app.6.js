...
function startGroupDrag(e, g, div) {
  // Corrección: el offset se calcula con el punto de pulsación, no la esquina.
  ...
  const onMove = ev => {
    ...
    g.x = w.x - offX; g.y = w.y - offY;
    div.style.left = g.x + "px"; div.style.top = g.y + "px";
    for (const n of state.notes) {
      if (n.group_id === g.id) {
        n.x = w.x - offX + (n.x - g.x); // mantener desplazamiento relativo
        n.y = w.y - offY + (n.y - g.y);
...
function createNoteEl(n) {
  ...
  anchorBtn.addEventListener("click", e => { e.stopPropagation(); const g = groupUnder(n); if (g) anchorToGroup(n, g.id); });
...
async function anchorToGroup(n, gid) { n.group_id = gid; await api(`/api/notes/${n.id}`, "PATCH", { group_id: gid }); renderAll(); applyFilter(); applyPrivacy(); }
...
function attachNoteEvents(div, n, anchorBtn) {
  ...
  div.addEventListener("pointerup", () => {
    ...
    if (g && n.group_id !== g.id) { anchorBtn.classList.remove("hidden"); anchorBtn.textContent = "📌 " + (g.title || "grupo"); }
    else anchorBtn.classList.add("hidden");
  });
}
...
function startNoteDrag(e, note, div) {
  ...
  const onMove = ev => {
    ...
    const w = screenToWorld(ev.clientX, ev.clientY);
    note.x = w.x - offX; note.y = w.y - offY;
...
el.createMenu.querySelectorAll(".create-item").forEach(item => {
  item.addEventListener("click", () => {
    ...
    if (type === "connect") {
      // Modo conectar: arrastra entre anclas.
      state.pendingType = null;
      el.connectHint.classList.remove("hidden");
      setTimeout(() => el.connectHint.classList.add("hidden"), 4000);
      closeCreateMenu();
      return;
    }
...
function createBookmarkRow(b) {
  ...
  if (b.favicon) { const f = document.createElement("img"); f.className = "bm-favicon"; f.src = b.favicon; f.alt = ""; f.onerror = () => f.remove(); row.appendChild(f); }
