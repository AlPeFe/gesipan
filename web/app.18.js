...
function createNoteEl(n) {
  ...
  if (n.group_id) { const a = document.createElement("span"); a.className = "note-anchored"; a.textContent = "📌"; a.title = "En un grupo"; div.appendChild(a); }
...
// Actualiza de forma determinista los botones de anclar/desanclar de todas las notas:
// - Nota seleccionada + en un grupo → "Desanclar"
// - Nota seleccionada + sobre un grupo (sin estar anclada) → "Anclar a X"
// - Cualquier otro caso → oculto
function updateAnchorButtons() {
  el.world.querySelectorAll(".note").forEach(div => {
    const btn = div.querySelector(".anchor-group-btn");
    if (!btn) return;
    const n = state.notes.find(x => x.id === Number(div.dataset.id));
    if (!n) return;
    const isSel = state.selectedNoteId === n.id;
    if (isSel && n.group_id) {
      btn.classList.remove("hidden");
      btn.textContent = "📌 Desanclar";
    } else if (isSel && !n.group_id) {
      const g = groupUnder(n);
      if (g) { btn.classList.remove("hidden"); btn.textContent = "📌 Anclar a " + (g.title || "grupo"); }
...
function drawConnections() {
  ...
  for (const c of state.connections) {
    ...
    // Seleccionable: si es la flecha seleccionada, resaltar.
    path.setAttribute("class", "conn" + (state.selectedConnectionId === c.id ? " selected" : ""));
    path.dataset.connId = c.id;
    // Clic en la flecha: la selecciona (para borrar con Supr).
...
document.addEventListener("keydown", e => {
  if (e.key === "Escape") {
    ...
    state.selectedConnectionId = null; drawConnections();
    setConnectMode(false); closeCreateMenu(); hideInspector();
  }
  // Supr / Delete borra lo seleccionado: nota, grupo o flecha.
  if ((e.key === "Delete" || e.key === "Backspace") && !e.target.isContentEditable && !e.target.matches("input,textarea")) {
...
// Borra una conexión (flecha) entre notas.
...
function hideInspector() {
  el.inspector.classList.add("hidden"); editingNote = null;
  // Al cerrar el detalle, la nota pierde el foco/selección.
...
// Botón "Eliminar grupo" del menú contextual.
const groupMenuDelete = document.getElementById("group-menu-delete");
groupMenuDelete.addEventListener("click", () => {
...
document.addEventListener("pointerdown", (e) => {
  const noteId = editingNote ? editingNote.id : null;
  if (noteId === null) return;
  const noteEl = el.world.querySelector(`.note[data-id="${noteId}"]`);
